...
public class kNN_GA {
  ...
  public static boolean[] GA() {
    //Adjusted the appropriate sizes for POP and Max Gen
    int POP_SIZE = 100; // population size - for assignment, see brief for limit
    ...
    for (int gen = 0; gen < MAX_GEN; gen++) { // do for many generations
      ...
      for (int j = 0; j < POP_SIZE; j++) {
        if (rn.nextDouble() < 0.8) // how many times, pcrossover=0.5
        {
      ...
      for (int j = 0; j < POP_SIZE; j++) {
        if (rn.nextDouble() < 0.15) { // how many times, pm=0.1
      ...
      // not that this line might need to removed (or kept) depending on how the GA
      // operators are coded
  ...
  public static double KNN(double[][] train, double[][] val, int[] train_label, int[] val_label, boolean[] sol) {
    ...
    int NUM_NEIGHBOUR = 9; // replace with optimal k value
